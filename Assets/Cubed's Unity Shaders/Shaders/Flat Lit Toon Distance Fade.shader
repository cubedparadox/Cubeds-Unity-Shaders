...
Shader "CubedParadox/Flat Lit Toon Distance Fade" {
    Properties {
        ...
        _DistanceStart ("Distance Start", Float ) = 1
        _DistanceEnd ("Distance End", Float ) = 0
    ...
    SubShader {
        ...
        Pass {
            ...
            float3 SphericalHarmonics( float3 normal ){
            ...
            float4 frag(VertexOutput i) : COLOR {
                float4 objPos = mul ( unity_ObjectToWorld, float4(0,0,0,1) );
                float node_6911 = distance(i.posWorld.rgb,_WorldSpaceCameraPos);
                ...
                clip((1.0 - (((node_6911-_DistanceEnd)/(_DistanceStart-_DistanceEnd))*lerp(1.0,node_8453,_DitherAmount))) - 0.5);
////// Lighting:
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(i.uv0, _MainTex));
                float4 _ColorMask_var = tex2D(_ColorMask,TRANSFORM_TEX(i.uv0, _ColorMask));
                float3 finalColor = (lerp((_MainTex_var.rgb*_Color.rgb),_MainTex_var.rgb,_ColorMask_var.r)*saturate((SphericalHarmonics( float3(0,1,0) )+CubemapReflections( normalize((_WorldSpaceCameraPos-objPos.rgb)) ))));
